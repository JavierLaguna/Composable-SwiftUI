...
Build app:
```bash
xcodebuild -project "Composable SwiftUI.xcodeproj" -scheme "Composable SwiftUI" -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=26.5' build
```

Build tests target/scheme:
```bash
xcodebuild -project "Composable SwiftUI.xcodeproj" -scheme "Composable SwiftUITests" -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=26.5' build
```

## Test Commands
Run all tests with xcodebuild:
```bash
xcodebuild -project "Composable SwiftUI.xcodeproj" -scheme "Composable SwiftUITests" -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=26.5' test
```
...
Run one test method:
```bash
xcodebuild -project "Composable SwiftUI.xcodeproj" -scheme "Composable SwiftUITests" -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=26.5' -only-testing:"Composable SwiftUITests/CharactersListReducerTests/getCharactersSuccess" test
```

Run one test type/file:
```bash
xcodebuild -project "Composable SwiftUI.xcodeproj" -scheme "Composable SwiftUITests" -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=26.5' -only-testing:"Composable SwiftUITests/CharactersListReducerTests" test
```
